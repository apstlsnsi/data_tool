...
def handle_missing_values(df, strategy='drop', columns=None, fill_value=None):
    ...
    # Если columns не указан, используем все столбцы
    if columns is None:
        columns = df_copy.columns
    
    # Если strategy - словарь, обрабатываем каждый столбец индивидуально
    if isinstance(strategy, dict):
        for col, col_strategy in strategy.items():
            if col not in df_copy.columns:
                continue
                
            # Получаем fill_value для этого столбца (если есть)
            col_fill_value = fill_value.get(col) if isinstance(fill_value, dict) else fill_value
            
            # Обрабатываем пропуски в столбце
    ...
    # Если strategy - строка, применяем ко всем столбцам
    if strategy == 'drop':
        return df_copy.dropna(subset=columns)
    
    # Обработка для всех указанных столбцов
    for col in columns:
        df_copy = _apply_missing_value_strategy(
            df_copy, 
            col, 
...
def _apply_missing_value_strategy(df, col, strategy, fill_value=None):
    ...
    # Если fill_val None, оставляем пропуски как есть
    if fill_val is None:
        return df
...
def clip_outliers(df, column, method='iqr', threshold=1.5, lower_quantile=0.05, upper_quantile=0.95):
    """
    ...
    column : str
        Column name
    method : {'iqr', 'quantile'}, optional
        Outlier detection method
    threshold : float, optional
        IQR multiplier (for 'iqr' method)
    lower_quantile : float, optional
        Lower quantile (for 'quantile' method)
    upper_quantile : float, optional
        Upper quantile (for 'quantile' method)
    ...
    df_copy[column] = series.clip(lower_bound, upper_bound)
    return df_copy
